...
public class Mocha extends JFrame implements ActionListener {
	JTextField mony,Qty;
        int n=0;
        JLabel MochaText;
	...
	JButton sum,Minus,result,Order,checkData;
	double size=0;
	Mocha(){
		...
		panel.add(Picture);
		ImageIcon image=new ImageIcon("Mocha1.png");
		Picture.setIcon(image);
		
		MochaText=new JLabel("Mocha");
		MochaText.setBounds(100, 290, 200, 50);
		MochaText.setFont(new Font("Ebrima",Font.BOLD,40));
		add(MochaText);
			...
			TextIce=little.getText();
                 ...
                 if(e.getSource()==Order){
                    ...
                    try (BufferedWriter writer = new BufferedWriter(new FileWriter("D:\\netbean\\Coffee\\src\\coffee\\order.txt", true))) {
                        ...
                        writer.write("Drink: " + MochaText.getText() + "\t");
                    ...
                    new ABA(mony.getText(),TextSize,Textsuger,TextIce,MochaText.getText(),String.valueOf(size),Qty.getText());
                    dispose();
                }
